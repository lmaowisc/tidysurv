...
### Apply the same plotting grammar to survival curves

The `ggsurvfit` package connects fitted survival estimates to `ggplot2`. The prepared `rfs` data already contain the required time, event indicator, and group labels. The following fits the same unadjusted curves as the standard analysis, then adds presentation layers.

```{r}
#| label: fig-tidy-km-introduction
#| fig-width: 8
#| fig-height: 6
#| fig-cap: "Relapse-free survival by hormone therapy, with confidence intervals and numbers at risk."
library(ggsurvfit)
...
The pipe passes the fitted object into `ggsurvfit()`. The `+` operators then add graphical components. `survfit2()` retains information about the fitting environment that helps produce readable group labels. These functions and their relationship to `ggplot2` are described in the [package introduction](https://www.danieldsjoberg.com/ggsurvfit/).

The shaded regions display confidence intervals. The risk table reports how many patients remain under observation and event-free at each displayed time. At the origin its counts are 440 and 246, matching the treatment-group totals. Smaller numbers later in follow-up help explain the greater uncertainty in late survival estimates.

`risktable_stats = "n.risk"` requests only the number at risk. Other choices include cumulative events (`"cum.event"`) and interval event counts (`"n.event"`); these are different quantities and require appropriate labels. The [risk-table reference](https://www.danieldsjoberg.com/ggsurvfit/reference/add_risktable.html) explains the available summaries. The axis breaks above also determine the default risk-table times.
...
The `statistic` argument specifies what each cell should display. `{median}`, `{p25}`, and `{p75}` stand for the computed median and quartiles. For a category, `{n}` is its count and `{p}` its percentage within the treatment column. The expressions with `~` associate variable selections with display instructions; they are table specifications rather than regression models.
...
### Report estimates from a fitted model

The same approach extends to regression output. Fit a model first, then pass the fitted object to `tbl_regression()`. Using the specification from the standard Cox analysis allows a direct comparison of the estimates.

```{r}
#| label: tidy-cox-table
...
- `exponentiate = TRUE` reports hazard ratios instead of log hazard coefficients for this Cox model.
- `include` selects terms to display. It does not remove the other covariates from the fitted model: the displayed results remain adjusted for all terms in the formula.
- `label` supplies readable descriptions and makes the units of continuous comparisons explicit.

The hormone-therapy estimate remains about 0.70, with a 95% confidence interval of approximately 0.54 to 0.90. Improved formatting does not change the estimate or resolve the diagnostic concerns identified in the original fit. The [official regression-table tutorial](https://www.danieldsjoberg.com/gtsummary/articles/tbl_regression.html) describes supported models and additional reporting options.
...
For competing risks, retain distinct event types rather than reusing the binary composite `event`. `tidycmprsk::cuminc()` expects an event-status factor with censoring as its first level; `ggsurvfit::ggcuminc()` can then display the cumulative incidence estimates. The [event-coding reference](https://mskcc-epi-bio.github.io/tidycmprsk/reference/cuminc.html) is important because a formatting change cannot correct an incorrectly defined endpoint.

Applying this approach to fitted survival models connects a prepared dataset to estimates, tables, and graphics in a reproducible sequence. [Chapter 3](np.qmd) develops this connection for nonparametric survival methods.
